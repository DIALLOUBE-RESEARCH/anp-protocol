...
export class MessageRouter {
     ...
     */
    static handleIncomingMessage(senderId: string, payload: ANPPayload, ws: WebSocket) {
        try {
            ...
            if (payload.intent === 'SUBSCRIBE' && payload.targetId) {
                ...
                registry.subscribe(senderId, payload.targetId);
     ...
     */
    private static routeDirectMessage(senderId: string, targetId: string, payload: ANPPayload) {
        const targetConn = registry.getConnection(targetId);
        if (!targetConn) {
        ...
        if (!registry.hasSubscription(targetId, senderId)) {
     ...
     */
    private static routeBroadcast(senderId: string, payload: ANPPayload) {
    ...
    private static sendAck(ws: WebSocket, message: string) {
