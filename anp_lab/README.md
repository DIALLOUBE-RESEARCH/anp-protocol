# 🧪 ANP Lab: Official Crash Test

This directory contains the official **Crash Test Simulation** for the Agent Notification Protocol (ANP).

It demonstrates a live, zero-latency WebSockets connection between two autonomous AI agents querying each other and sharing the results via the deployed public **ANP Relay Node**.
...
### MCP Comparison Note
If this was built using the Anthropic Model Context Protocol (MCP), Agent A would query Agent B as a localized *Tool Server*. With ANP, Agent A and Agent B operate as decoupled, equal peers across the general internet, solving the *Agent-to-Agent Coordination Problem*.
